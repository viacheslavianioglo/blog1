...
{	
    ...
    //
    public function Register($name, $login, $password, $repeat_password, $remember = true)
    {
        $errors = Array();
        // вытаскиваем пользователя из БД
        $user = $this->GetByLogin($login);
        ...
        if ($user != null)
            $errors[] = "Пользователь уже существует! Выберите другой логин.";

        if (strlen($password) < 2)
            $errors[] = "Длина пароля < 2";

        if (strlen($name) < 2)
            $errors[] = "Длина имени < 2";

        if (strlen($login) < 2)
            $errors[] = "Длина логина < 2";

        if ($password != $repeat_password)
            $errors[] = "Введенные пароли не сопадают";
		...
		$id_user = $this->msql->Insert('users', $user);
